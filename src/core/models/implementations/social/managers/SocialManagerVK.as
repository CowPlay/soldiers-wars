...
import core.models.implementations.social.vk.APIConnection;
...
public class SocialManagerVK extends SocialManagerBase
{
    ...
    private function onInitSocialUserInfoComplete(response:*):void
    {
        ...
        _userInfo = getUserInfoFromResponse((response as Array)[0], true);
    ...
    private function onFriendsGetComplete(response:*):void
    {
        ...
        {
            _friendsListAll.push(getUserInfoFromResponse(response[i], false));
        }
    ...
    private function onInitFriendsInstalledComplete(response:*):void
    {
        ...
        Debug.assert(false, "Update friends all -> isInstalled");
    ...
    private static function getUserInfoFromResponse(response:*, isInstalled:Boolean):SocialUserInfo
    {
        var id:String = response.hasOwnProperty("uid") ? String(response.uid) : "";
        var firstName:String = response.hasOwnProperty("first_name") ? response.first_name : "";
        var lastName:String = response.hasOwnProperty("last_name") ? response.last_name : "";

        //load picture
        var urlPicture:String = response.hasOwnProperty("photo_100") ? response.photo_100 : null;

        return SocialUserInfo.createSocialUserInfo(id, firstName, lastName, isInstalled, urlPicture);
    }
...
//        transferItem = "euro" + currency.toString();
//
